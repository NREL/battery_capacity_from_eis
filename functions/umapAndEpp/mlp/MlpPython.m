...
classdef MlpPython
    ...
    % dependencies.  If Python version 3.7, 3.8 or 3.9 is not
    % present the function guides the user to the website for version
    % 3.7.9.  If TensorFlow dependencies are missing the function 
    % explains the imports needed and lets the user choose automatic 
    % installation.  The imports needed are: pandas, numpy, matplotlib, 
    % sklearn and tensorflow.  The main processing is down through our 
    % script mlp.py which you are free to evolve.
    %
    % By default we invoke Python indirectly using the system command which
    % calls either mlpTrain.py or mlpPredict.py to marshall input and ouput
    % files and other command line arguments.  If you are using MATLAB 
    % r2019b or later we avoid command line processing and csv file 
    % processing and invoke the mlp_predict2 function in mlp.py directly 
    % in process using MATLAB's py routines.  For MATLAB py functions to
    % work you must call pyenv (only once) immediately after MATLAB starts 
    % to identify where the Python with TensorFlow is located.  
    % For example on a Mac for version 3.7.9 the command is likely
    ...
    %   function's header.  No folder given for CSV file inputs arguments =
    %   defaults to ~/Documents/run_umap/examples folder.  If they don't exist'
    %   they are downloaded from our server.
    %
    %   1.      Build similar model as run_umap's example 33 without umap.
    %           The training progress is reported in an OS "shell window"
    %           (terminal on MAC and cmd.exe on MS Windows).
    %           The sample trained on is a balbc mouse strain.
    %           Then classify a separate sample with a different staining
    %           preparation (Fmo=fluoresence minus one).  
    ...
    %
    %   2.      Build similar model as run_umap's example 39 without umap.
    %           The training progress is reported in an OS "shell window"
    %           (terminal on MAC and cmd.exe on MS Windows) but unlike the 
    %           above example 1 MATLAB does not wait for it to complete.
    %           The biological sample trained on is the same as example 1 
    %           but with an additional stain measurements.
    %           Then classify on a biological sample with compatible measurements 
    %           but from a different mouse strain that has is genetically 
    %           modified to lack T cells or B cells.   
    %           Then show the Hi-D match result.
    %
    ...
    %
    %   3.      Repeat example 2's Predict call except provide the data
    %           matrix directly.
...
%                           matrix otherwise the column names are 
%                           required in the first line of the csv file.
%
...
%       =Mlp.Predict(csvFileOrData, 'NAME1',VALUE1, 'NAMEN',VALUEN) 
%            
%RETURN VALUES
%   Invoking Mlp.Predict produces 5 return values:
...
%   matrix if invoiked by the system command.
...
%      Every column represents a traihining class.
%   6)qfTable: the match table object produced if the 
%
%   REQUIRED INPUT ARGUMENT
%   csvFileOrData is a CSV file containing a matrix or a matrix where the
%   columns are numeric measurements.  If the last column contains 
%   numeric identifiers for an alternate classification of the matrix row 
%   THEN the argument has_labels must be true. Predict then produces 
%   a Hi-D Match table object if invoked with a 5th argout.
%
%   OPTIONAL NAME VALUE PAIR ARGUMENTS
%
%   Name                    Value
...
%                           required in the first line of the csv file.
%
%   'model_file'            Name of file to load the model from.  or a reference 
%                           to th ClassificationNeuralNetwork object 
%                          If no argument is specified then this function 
%                           uses the csvFile
%                           folder and .
...
%                           If no folder for the file is given then 
%                           Train  will query for its location.
%                   
%                           
