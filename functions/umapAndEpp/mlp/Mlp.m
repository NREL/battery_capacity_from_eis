...
%   they are downloaded from ourr server.
%
%   1.      Build similar model as run_umap's example 33 without umap.
%           The training progress is reported in MATLAB's Command Window.
%           Then classify a separate sample.  Then show the Hi-D match result.
%
...
%
%   2.      Build similar model as in example 1, except:  a)pass training data in as a
%           a numeric matrix;b) hold out 33% of the rows for validation.
%           c)give it a different name; d) get the MATLAB object 
%           (ClassificationNeuralNetwork) as the 2nd argout and use it as the 
%           model_file argument to Predict and e)Acceleration==none.
%
...
%
%
%   3.      Build similar model as run_umap's example 39 without umap.
%           The training progress is reported in MATLAB's Command Window.
%           Then classify a separate sample.  Then show the Hi-D match result.
%
...
%                           to the folder containing the input CSV file.
...
%   'class_limit'           0 to 1 to check the max percrentage of classes
%                          to matrix rows to reasonable.
%                           Default is .1.  Thus if for a matrix
%                           of 400 rows the unique values in the label
%                           column must NOT exceed 40.
%
...
% %Mlp.Predict classifies a data matrix using a prior neural networks 
% built by MATLAB's fitcnet module that they introduced in release r2021a.
%
...
%   3)model:  a reference to the MATLAB fitcnet object used ClassificationNeuralNetwork
...
%   5)qfTable: the match table object produced if the 
%
%   REQUIRED INPUT ARGUMENT
